...
QUERY_MODEL_LOCAL_PATH = 'C:/Users/USER/.cache/huggingface/hub/models--meta-llama--Meta-Llama-3-8B-Instruct/snapshots/e1945c40cd546c78e41f1151f4db032b271faeaa'

# Load the tokenizer and model for embeddings
vector_model = SentenceTransformer(VECTOR_MODEL_NAME)

# Load the LLaMA 3 model for chat from the local path
chat_tokenizer = AutoTokenizer.from_pretrained(QUERY_MODEL_LOCAL_PATH)
chat_model = AutoModelForCausalLM.from_pretrained(QUERY_MODEL_LOCAL_PATH)
...
def query_vector_database(query, top_k=2):
    inputs = vector_model.encode(query, convert_to_tensor=True)
    response = index.query(vector=inputs.numpy().tolist(), top_k=top_k)
...
# Function to generate chat response using LLaMA 3
def generate_chat_response(query, context):
    input_text = f"{context}\n\n{query}"
    inputs = chat_tokenizer(input_text, return_tensors='pt')
    outputs = chat_model.generate(**inputs, max_length=512, num_return_sequences=1)
...
def main():
    ...
    final_response = generate_chat_response(query, prompt)
